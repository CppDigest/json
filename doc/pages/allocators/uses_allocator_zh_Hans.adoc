...
To support code bases which are already using polymorphic allocators, the containers in this library support {std_uses_allocator} construction. For <<ref_array>>, <<ref_object>>, <<ref_string>>, and <<ref_value>>:

* The nested type `allocator_type` is an alias for a {ref_polymorphic_allocator}.

* All eligible constructors which accept <<ref_storage_ptr>> will also accept an instance of {ref_polymorphic_allocator} in the same argument position.
