...
= 基准测试
本节将 Boost.JSON 与两个广泛使用的功能相似库进行性能比较：以性能著称的 RapidJSON 和以功能丰富见长的 JSON for Modern C++。基准测试程序通过解析和序列化一组代表典型工作负载的 JSON 来测量吞吐量。评估的实现方案包括：
...
| *boost(pool)*
| 使用 <<ref_monotonic_resource>> 解析输入，该资源池专为无需后续修改的解析场景优化。<<ref_stream_parser>> 对象在多次测试间复用，使实现分配的临时内存得以保留从而提升性能。

| *boost*
| 使用 <<default_memory_resource, 默认内存资源>> 解析输入，该资源采用标准 C++ 分配器，设计用于包括文档解析后修改在内的通用场景。<<ref_stream_parser>> 对象在多次测试间复用，使实现分配的临时内存得以保留从而提升性能。

| *rapidjson(pool)*
| 使用 https://rapidjson.org/classrapidjson_1_1_memory_pool_allocator.html[`MemoryPoolAllocator`] 实例解析输入，该分配器专为无需后续修改的解析场景优化。存储临时内存的 https://rapidjson.org/classrapidjson_1_1_generic_document.html[`Document`] 对象不在测试间复用，否则内存消耗将无限增长导致基准测试失效。

| *rapidjson*
| 使用 https://rapidjson.org/classrapidjson_1_1_crt_allocator.html[`CrtAllocator`] 实例解析输入，该分配器采用标准 C++ 分配器，设计用于包括文档解析后修改在内的通用场景。存储临时内存的 https://rapidjson.org/classrapidjson_1_1_generic_document.html[`Document`] 对象不在测试间复用，否则内存消耗将无限增长导致基准测试失效。

| *nlohmann*
| 使用静态成员函数 https://nlohmann.github.io/json/classnlohmann_1_1basic__json_ab330c13ba254ea41fbc1c52c5c610f45.html[`json::parse`] 解析输入，该函数使用默认 `std` 分配器，设计用于包括文档解析后修改在内的通用场景。该库未提供在后续操作中复用解析或序列化期间临时存储的接口。
|===

== 方法论
首先加载所有输入文件。随后对每种配置运行足够次数的测试，确保总耗时至少 5 秒。系统记录耗时、调用次数（解析或序列化）及传输字节数作为样本，同时计算以兆字节/秒为单位的吞吐量。每种配置生成若干样本（当前为五个），剔除除中位数附近两个样本外的所有数据，剩余样本取平均值后作为基准测试结果上报。

输入文件存放于 bench/data 目录，具体分布如下：
...
| 2.2MB
| 最大文件，包含大量存储浮点坐标对的二元数组。

| <<parse_citm_catalog_json,*citm_catalog.json*>>
| 1.69MB
| 包含多样化嵌套结构、类型及长度的 JSON 文件。
...
| 550KB
| 移除空格并将非 ASCII 字符替换为 Unicode 转义序列的 twitter.json 文件。
...
测试硬件：**Intel(R) Core(TM) i7-7700K CPU @ 4.20GHz**，Windows 10 系统，32GB 内存。
